...
def load_images_from_folder(folder, target_size=(IMG_HEIGHT, IMG_WIDTH)):
    ...
    Images are resized to target_size, normalized to [0,1],
    and converted from HWC to CHW format.
    """
    images = []
    for filename in os.listdir(folder):
        ...
        try:
            with Image.open(img_path) as img:
                ...
                img = np.array(img)
                # If an image has an alpha channel, drop it.
                if img.shape[-1] == 4:
                    img = img[..., :3]
...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        # Encoder: reduce spatial dimensions from 1024x1024 to 16x16
        self.encoder = nn.Sequential(
            nn.Conv2d(IMG_CHANNELS, 32, kernel_size=3, padding=1),   # 1024x1024x32
            nn.ReLU(),
            ...
            nn.Conv2d(512, 1024, kernel_size=3, padding=1),            # 32x32x1024
            nn.ReLU(),
            nn.MaxPool2d(2, stride=2)                                  # 16x16x1024
        )
        # Decoder: upsample step by step back to 1024x1024
        self.decoder = nn.Sequential(
            nn.Conv2d(1024, 1024, kernel_size=3, padding=1),           # 16x16x1024
            nn.ReLU(),
            nn.Upsample(scale_factor=2, mode='nearest'),               # 32x32x1024

            nn.Conv2d(1024, 512, kernel_size=3, padding=1),            # 32x32x512
            nn.ReLU(),
...
def plot_and_save_reconstructions(original, reconstructed, n=10, save_path="reconstructed_vs_original.png"):
    ...
    Both 'original' and 'reconstructed' are numpy arrays with shape (N, channels, H, W);
    ...
    for i in range(n):
        ...
        # Row 3: Residual heatmap (absolute difference)
...
def compute_reconstruction_errors(model, images, device):
    ...
    Returns an array of errors (one per image) and the corresponding reconstructions.
    """
    model.eval()
    errors = []
    reconstructions = []
...
def main():
    ...
    train_folder = "./carpet/train/good"  # Training folder with normal images
    test_folder = "./carpet/test/cut"      # Test folder with potential anomalies
    ...
    # Create a TensorDataset and split training data into train and validation sets (90%/10%)
    dataset = TensorDataset(x_train, x_train)  # Input and target are the same
    total_samples = len(dataset)
    n_val = int(0.1 * total_samples)
    n_train = total_samples - n_val
    train_dataset, val_dataset = random_split(dataset, [n_train, n_val])
    
    batch_size = 16  # Adjust based on your system capabilities
