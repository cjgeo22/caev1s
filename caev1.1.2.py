...
def build_autoencoder(input_shape=(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS)):
    """
    Build a deeper convolutional autoencoder with 6 sets of convolutional layers.
    The encoder compresses the image down to 16x16, and the decoder reconstructs
    the image back to 1024x1024.
    """
    ...
    x = Conv2D(1024, (3, 3), activation='relu', padding='same')(x)              # 32x32x1024
    encoded = MaxPooling2D((2, 2), padding='same')(x)                           # 16x16x1024
    
    # Decoder: reconstruct the image back to 1024x1024
    x = Conv2D(1024, (3, 3), activation='relu', padding='same')(encoded)        # 16x16x1024
    x = UpSampling2D((2, 2))(x)                                                  # 32x32x1024
    
    x = Conv2D(512, (3, 3), activation='relu', padding='same')(x)               # 32x32x512
...
def main():
    ...
    test_folder = "./carpet/test/cut"      # Test folder
    ...
    # Build the deep convolutional autoencoder with 6 sets of layers
